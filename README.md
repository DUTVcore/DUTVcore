# Hi there, I'm DUTVcore! ⚡🛠️

### 👨‍💻 Computer Engineering Student @ DUT
> **"Underwear can rip, but the PC must be maxed out (Max Performance)!"**

Hardware Enthusiast, Overclocking Addict, and System Optimization Freak.

---

### 🚀 Future Goals (Maybe in the Future)
**Target:** Become an **Extreme Overclocker**, Hardware Modder, and Silicon Lottery Player.
**Mission:** Pushing silicon to its absolute limit. Aiming to shatter World Records on **HWBOT** (Cinebench, 3DMark, SuperPi).

---

### 🖥️ MY MAIN RIG (The Monster)
*Main workstation configuration*

| Component | Specification |
| :--- | :--- |
| **Motherboard** | ROG MAXIMUS **Z890 HERO** |
| **CPU** | Intel® Core™ **Ultra 9 Processor 285K** |
| **GPU** | Colorful iGame **RTX 5090 D** Vulcan OC |
| **RAM** | Trident Z5 RGB **256GB** (4x64GB) DDR5-6000 CL34 |
| **Cooling** | AIO Valkyrie B360-GT Black |
| **PSU** | Asus ROG STRIX **1200P** Gaming Platinum (ATX 3.1) |
| **Case** | SAMA 4503 *(I still can't believe I fit 4 HDDs in here!)* |

#### 💾 Storage System (Total Capacity: ~124 TB)
> *"Data is currency, Speed is life."*

* **OS/Boot (Gen 5 Speed):**
    * 2x SSD Samsung **9100 Pro 8TB** (PCIe Gen 5.0 x4) - *Raid 0*
* **High-Speed Workspace:**
    * 4x SSD Kingston KC3000 **4TB** (NVMe Gen 4)
* **The Vault (Mass Storage):**
    * 2x HDD WD Ultrastar **22TB** HC570 (Data Center Grade)
    * 1x HDD Ultrastar HC680 **28TB**
    * 1x HDD Seagate SkyHawk AI **20TB**

#### 🎮 Peripherals
* **Primary Monitor:** Viewsonic XG251G 25” **360Hz** IPS
* **Secondary Monitor:** MSI G244F **170Hz** IPS
* **Gears:** Razer Viper V3 PRO | ATK M1 | ROG Cetra II Core | Pad ATK XSoft

---

### 🛠️ Tech Stack & Tools
<p align="left">
  <img src="https://img.shields.io/badge/C%2B%2B-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white" />
  <img src="https://img.shields.io/badge/Hardware-FF0000?style=for-the-badge&logo=asus&logoColor=white" />
  <img src="https://img.shields.io/badge/Overclocking-FFA500?style=for-the-badge&logo=intel&logoColor=white" />
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/Windows_11-0078D6?style=for-the-badge&logo=windows&logoColor=white" />
  <img src="https://img.shields.io/badge/Visual_Studio_Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white" />
</p>

---

<div align="center">
  <img src="https://github.com/platane/snk/raw/output/github-contribution-grid-snake.svg" alt="Snake animation" />
</div>
